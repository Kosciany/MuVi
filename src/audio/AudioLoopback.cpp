...
namespace Muvi {
    ...
    AudioLoopback::AudioLoopback(void) {
        ...
        {
            ffuint *mix_format = (ffuint *) this->audio_interface->dev_info(current_dev, FFAUDIO_DEV_MIX_FORMAT);
        ...
        result = this->audio_interface->open(this->audio_buf , &this->audio_conf, FFAUDIO_LOOPBACK);
        ...
        if (result) {
            MUVI_AUDIO_CRITICAL("Critical error occurred during initialization: {0}", result);
        }
    }
    ...
    void AudioLoopback::FillInternalBuffer(size_t size, T *input_buffer) {
        ...
        for (int j=0, i=0; j < size/(sizeof(T) * this->audio_conf.channels); j++) {
            this->samples_buff_chnl_1.push_back((float) *(ptr+i));
            i += this->audio_conf.channels;
        }

        if (this->audio_conf.channels > 1) {
            for(int j=0, i=(this->audio_conf.channels-1); j < size/(sizeof(T) * this->audio_conf.channels); j++) {
    ...
    void AudioLoopback::GetNewSamples(void){
        int result = this->audio_interface->read(this->audio_buf, (const void**) &this->buffer);

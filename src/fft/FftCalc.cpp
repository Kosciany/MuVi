...
namespace Muvi {
    ...
    void fft(float *x_in, std::complex<double> *x_out, int N) {
        ...
        for (int k = 0; k < CHUNK_SIZE; k++) {
            ...
            for (int n = 0; n < CHUNK_SIZE; n++) {
                output->amplitude_1[k] =
                    (real(output->amplitude_1[k]) +
                     input->first_channel[n] * cos(2 * M_PI * k * n / N)) +
                    (imag(output->amplitude_1[k]) -
                     input->first_channel[n] * sin(2 * M_PI * k * n / N)) *
                        1i;
            }
        }
        return;
